...
type Props = { session: Session | null; topUrl?: string };

export function LpContent({ session, topUrl = "/" }: Props) {
    ...
    <main style={{ background: "#000", minHeight: "100vh", color: "#fff" }}>
      <header className={styles.globalHeader}>
        <div className={styles.globalHeaderInner}>
          <Link href={topUrl} className={styles.globalHeaderLogo}>
            KIZASHI
          </Link>
          <nav className={styles.globalHeaderNav}>
            <Link href={topUrl} className={styles.globalHeaderLink}>トップ</Link>
